...
def convert_disps_to_depths_kitti(gt_disparities, pred_disparities):
    ...
    for i in range(len(gt_disparities)):
        ...
        pred_disp = (width / 2) * cv2.resize(pred_disp, (width, height), interpolation=cv2.INTER_LINEAR)
        pred_disparities_resized.append(pred_disp) 

        mask = gt_disp > 0

        # eigen baseline: 0.53272545, kitti baseline: 0.54
        gt_depth = width_to_focal[width] * 0.53272545 / (gt_disp + (1.0 - mask))
        pred_depth = width_to_focal[width] * 0.53272545 / pred_disp
